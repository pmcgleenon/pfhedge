...
from pfhedge.nn.functional import lookback_payoff_fixed, lookback_payoff_floating
...
class LookbackOption(BaseDerivative, OptionMixin):
    ...
    .. seealso::
        - :func:`pfhedge.nn.functional.lookback_payoff_fixed`
    ...
    def payoff_fn(self) -> Tensor:
        return lookback_payoff_fixed(self.ul().spot, call=self.call, strike=self.strike)
...
class LookbackOptionFloating(BaseDerivative, OptionMixin):
    r"""Lookback option with floating strike.

    The payoff of a lookback call option is given by

    .. math::
        \mathrm{payoff} = \max(S_T - \mathrm{Min}, 0) ,

    where
    :math:`S_T` is the spot price of the underlying at maturity and
    :math:`\mathrm{Min}` is the minimum of the underlier's spot price until maturity.

    The payoff of a lookback put option is given by

    .. math::
        \mathrm{payoff} = \max(\mathrm{Max} - S_T, 0) ,

    where
    :math:`S_T` is the spot price of the underlying at maturity and
    :math:`\mathrm{Max}` is the maximum of the underlier's spot price until maturity.

    .. seealso::
        - :func:`pfhedge.nn.functional.lookback_payoff_floating`
    ...
    Examples:
        >>> import torch
        >>> from pfhedge.instruments import BrownianStock
        >>> from pfhedge.instruments import LookbackOptionFloating
        >>>
        >>> _ = torch.manual_seed(42)
        >>> derivative = LookbackOptionFloating(BrownianStock(), maturity=5/250)
        >>> derivative.simulate(n_paths=2)
        >>> derivative.underlier.spot
        tensor([[1.0000, 1.0016, 1.0044, 1.0073, 0.9930, 0.9906],
                [1.0000, 0.9919, 0.9976, 1.0009, 1.0076, 1.0179]])
    ...
    def payoff_fn(self) -> Tensor:
        return lookback_payoff_floating(self.ul().spot, call=self.call)


# Assign docstrings so they appear in Sphinx documentation
_set_attr_and_docstring(LookbackOptionFloating, "simulate", BaseDerivative.simulate)
_set_attr_and_docstring(LookbackOptionFloating, "to", BaseDerivative.to)
_set_attr_and_docstring(LookbackOptionFloating, "ul", BaseDerivative.ul)
_set_attr_and_docstring(LookbackOptionFloating, "list", BaseDerivative.list)
_set_docstring(LookbackOptionFloating, "payoff", BaseDerivative.payoff)
_set_attr_and_docstring(LookbackOptionFloating, "moneyness", OptionMixin.moneyness)
_set_attr_and_docstring(LookbackOptionFloating, "log_moneyness", OptionMixin.log_moneyness)
_set_attr_and_docstring(
    LookbackOptionFloating, "time_to_maturity", OptionMixin.time_to_maturity
)